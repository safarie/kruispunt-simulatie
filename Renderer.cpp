...
void Renderer::recreateSwapChain()
{
    ...
    while (width == 0 || height == 0) {
        glfwGetFramebufferSize(ptr_window->get(), &width, &height);
        glfwWaitEvents();
    }
...
void Renderer::createDescriptorSetLayout()
{
    ...
    std::array<VkDescriptorSetLayoutBinding, 3> bindings = { duboLayoutBinding, uboLayoutBinding, samplerLayoutBinding };
...
void Renderer::createDescriptorSets()
{
    ...
    for (size_t i = 0; i < swapChainImages.size(); i++) {
        ...
        dynamicUniformBufferInfo.range = sizeof(DynamicUniformBufferObject);
        ...
        descriptorWrites[0].pBufferInfo = &dynamicUniformBufferInfo;
...
void Renderer::drawFrame() {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || ptr_window->framebufferResized) {
        ptr_window->framebufferResized = false;
        recreateSwapChain();
    }
...
// functions for selecting physical devices
